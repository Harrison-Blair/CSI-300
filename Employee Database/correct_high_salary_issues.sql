...
select eh.eid, eh.pos_id, NOW(),
	salary - (eh.salary - saHigh.sal_value) as "New Salary", 'One-time adjustment approved by management' as comments, 121212 as status_id
from EmpHistory eh join Sal_AdjustmentHigh saHigh on eh.pos_id = saHigh.pos_id
where (eh.salary - saHigh.sal_value) > 0
order by eh.eid;
...
*/
/*
select eh.eid, eh.pos_id, eh.salary, sah.sal_value,
	(eh.salary - sah.sal_value) as "Difference"
from EmpHistory eh join Sal_AdjustmentHigh sah on eh.pos_id = sah.pos_id
where eh.salary > sah.sal_value
order by eh.eid
*/